...
<section class="py-4 py-lg-5">
    <div class="container">
        ...
        <div class="card border-0 bdd_body">
            <h3>{{$businessService['name']}}</h3>
            {{-- <p class="d-flex">
                <span class="mr-5">START DATE<b class="d-block">8 Jun - 18 Jul' 22</b></span>
            ...
            <ul class="bl_tag mt-3 mb-4">
                <li>{{App\Models\BusinessType::where('id',$businessService['type_id'])->get('name')[0]->name}}</li>
                 {{-- <li>Certified</li>
                 <li>Trainings</li> --}}
            </ul>
            <p>
            ...
            <div class="broker_div mt-5">
                ...
                @if($businessService['user_id'] != Illuminate\Support\Facades\Auth::guard('user')->user()->id)
                <div>
                    <a class="btn bide_btn" href="{{route('user.bid.create',[$businessService['id']])}}">Bide Now</a>
                </div>
                @endif
            </div>
        </div>
