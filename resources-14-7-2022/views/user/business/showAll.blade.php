            ...
            <div class="left-part col-12 col-lg-4 pr-lg-5">
             ...
             <div class="right-part col-12 col-lg-8 pl-lg-5 border-left">
                ...
                <a href="{{route('user.businessService.show',[$bs->id])}}">
                    <div class="card">
                        <h3>{{$bs->name}}</h3>
                        <p>{{strlen($bs->description)>200?substr($bs->description,0,200).'...':$bs->description}} </p>
                        <h6>Rs. <span>{{number_format($bs->valuation)}}</span></h6>
                        <div class="d-flex justify-content-between align-items-center mt-4">
                            <div>
                                <ul class="bl_tag">
                                    <li>{{App\Models\BusinessType::where('id',$bs->type_id)->get('name')[0]->name}}</li>
                                </ul>
                            </div>
                            <div>
                                <a href="{{route('user.businessService.show',[$bs->id])}}">View details <i class="fas fa-angle-right"></i></a>
                            </div>
                        </div>
                        @if($bs->status==0)
